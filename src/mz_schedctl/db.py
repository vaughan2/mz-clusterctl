...
class Database:
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                try:
                    ...
                    logger.error("Error executing SQL", extra={'sql': sql, 'params': None, 'error': str(e)}, exc_info=True)
                    raise
                clusters = []
                
                for row in cur.fetchall():
                    logger.debug("Processing cluster row", extra={'row': row, 'row_types': {k: type(v).__name__ for k, v in row.items()}})
                    cluster = ClusterInfo.from_db_row(row)
                    logger.debug("Created ClusterInfo", extra={'cluster_id': cluster.id, 'cluster_id_type': type(cluster.id).__name__, 'cluster_name': cluster.name})
                    ...
                    params = (cluster.id,)
                    logger.debug("Executing SQL", extra={'sql': sql, 'params': params, 'param_types': [type(p).__name__ for p in params]})
                    try:
                        cur.execute(sql, params)
                        logger.debug("SQL executed successfully", extra={'rowcount': cur.rowcount})
                        replica_names = [replica_row['name'] for replica_row in cur.fetchall()]
                        ...
                        logger.error("Error executing SQL", extra={'sql': sql, 'params': params, 'error': str(e)}, exc_info=True)
                        raise
    ...
    def get_strategy_configs(self, cluster_id: Optional[UUID] = None) -> List[StrategyConfig]:
        ...
        logger.debug("Starting get_strategy_configs", extra={'cluster_id': str(cluster_id) if cluster_id else None})
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                if cluster_id:
    ...
    def get_strategy_state(self, cluster_id: str) -> Optional[StrategyState]:
        ...
        logger.debug("Starting get_strategy_state", extra={'cluster_id': cluster_id, 'cluster_id_type': type(cluster_id).__name__})
        with self.get_connection() as conn:
            with conn.cursor() as cur:
    ...
    def upsert_strategy_state(self, state: StrategyState):
        ...
        logger.debug("Starting upsert_strategy_state", extra={'cluster_id': state.cluster_id, 'state_version': state.state_version})
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                sql = """
    ...
    def log_action(self, cluster_id: str, action_sql: str, decision_ctx: Dict[str, Any], 
                   executed: bool, error_message: Optional[str] = None) -> str:
        """Log an action to the audit table"""
        action_id = str(uuid4())
        logger.debug("Starting log_action", extra={'action_id': action_id, 'cluster_id': cluster_id, 'executed': executed})
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                sql = """
    ...
    def wipe_strategy_state(self, cluster_id: Optional[UUID] = None):
        ...
        logger.debug("Starting wipe_strategy_state", extra={'cluster_id': str(cluster_id) if cluster_id else None})
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                if cluster_id:
