...
def get_cluster_signals(conn: psycopg.Connection, cluster_id: str, cluster_name: str) -> Signals:
    """
    ...
    """
    logger.debug("Starting get_cluster_signals", extra={'cluster_id': cluster_id, 'cluster_name': cluster_name})
...
def _get_replica_count(conn: psycopg.Connection, cluster_name: str) -> int:
    ...
    with conn.cursor() as cur:
        ...
        try:
            ...
            logger.debug("Replica count retrieved", extra={'cluster_name': cluster_name, 'count': count})
            return count
    ...
    with conn.cursor() as cur:
        ...
        try:
            ...
            if result and result['last_activity']:
                logger.debug("Last activity found", extra={'cluster_id': cluster_id, 'last_activity': result['last_activity']})
...
def _get_hydration_status(conn: psycopg.Connection, cluster_name: str) -> Optional[str]:
    ...
    with conn.cursor() as cur:
        ...
        logger.debug("Hydration status calculated", extra={
            'cluster_name': cluster_name, 
