...
class BurstStrategy(Strategy):
    ...
    def validate_config(self, config: Dict[str, Any]) -> None:
        ...
        required_keys = ['max_replicas', 'scale_up_threshold_ms', 'cooldown_s', 'idle_after_s']
    ...
    def decide(self, current_state: StrategyState, config: Dict[str, Any], 
               signals: Signals) -> List[Action]:
            ...
            if (now - last_decision).total_seconds() < cooldown_seconds:
                logger.debug(
                    ...
                    extra={
                        ...
                        'cooldown_remaining': cooldown_seconds - (now - last_decision).total_seconds()
                    }
                )
                return actions
        ...
        if signals.seconds_since_activity and signals.seconds_since_activity > idle_after_s:
    ...
    def next_state(self, current_state: StrategyState, config: Dict[str, Any],
                   signals: Signals, actions_taken: List[Action]) -> StrategyState:
        ...
        replicas_added = sum(action.expected_state_delta.get('replicas_added', 0) for action in actions_taken)
        replicas_removed = sum(action.expected_state_delta.get('replicas_removed', 0) for action in actions_taken)
